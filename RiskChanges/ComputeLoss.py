...
def ComputeLoss(con, exposureid, lossid, computecol='counts', **kwargs):
    ...
    loss['loss_id'] = lossid
    filedir=con+'/'+'LossResults'
    loss_table='Loss_'+str(lossid)+'.csv'
    writemeta.writelossmeta(con,lossid,filedir,loss_table,adminpk,earid,hazid,exposureid,computecol)
    writevector.writeLoss(loss, filedir, loss_table)
