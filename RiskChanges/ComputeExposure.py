...
from .RiskChangesOps import writemeta
...
def ComputeExposure(con, earid, hazid, expid, **kwargs):
    ...
    if (pop_col != None and pop_col != ''):
        ...
        print("population colume is not linked!")
        pop_col = 'pop_col'
    ...
    #this is fundamentally different for the desktop version
    exposure_table='Exposure_'+str(expid)+'.csv'
    filedir=con+'/'+'ExposureResults'
    writemeta.writeexposuremeta(con,expid,filedir,exposure_table,earid,hazid,Ear_Table_PK,adminpk)
    writevector.writeexposure(df, filedir,exposure_table)
