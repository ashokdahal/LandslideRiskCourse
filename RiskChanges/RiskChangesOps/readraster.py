...
def readhaz(connstr, hazid):
    ...
    hazfile = hazard_metadata.file_name[0]
    hazdir = hazard_metadata.filedir[0]
    ...
    if intensity_type == 'Susceptibility':
        outfile = hazdir+'/'+hazfile
    else:
        hazpath=hazdir+'/'+hazfile
        outfile = ClassifyHazard(hazpath, base, step_size, threshold)
