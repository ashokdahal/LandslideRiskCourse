from RiskChanges.RiskChangesOps import writemeta
...
def combineLosses(lossess, interacting_hazards, interaction='independent', haz_prob=None):
    if interaction == 'cascading':
        ...
        raise ValueError(
            "The interaction names do not match the possible hazard interaction. It can be only independent, compounding, cascading, conditional or coupled")
    ...
    for col in cols:
        ...
        combined_loss_step = combined_loss_step.rename(
            columns={'combined': new_name})
...
def computeMulRisk(connstr, groupcombinations, extensions, riskid, **kwargs):
    ...
    for group in groupcombinations:
                ...
                for single_interaction in interacting_hazards:
                    ...
                    if second:
                        ...
                        for col in cols_selected:
                            merged_selected = [
                                word for word in merged_cols if (col in word)]
        ...
        if first_risk:
            ...
            combined_df = pd.merge(left=totalrisk, right=risk, how='outer', left_on=[
                                   'Unit_ID'], right_on=['Unit_ID'], right_index=False)
            combined_df["AAL_sum"] = combined_df["AAL_total"] + \
                combined_df["AAL"]
    ...
    risk['risk_id'] = riskid
    filedir=connstr+'/'+'RiskResults'
    risk_table='Risk_'+str(riskid)+'.csv'
    if not onlyaggregated:
        adminpk=999999
        writemeta.writeriskmeta(riskid,filedir,risk_table,adminpk)
        writevector.writeRisk(risk, filedir, risk_table)
    if is_aggregated:
        ...
        risk['risk_id'] = riskid
        writemeta.writeriskmeta(riskid,filedir,risk_table,adminpk)
        writevector.writeRiskAgg(risk, filedir, risk_table)
